...
export let ufload = (fileName) => {
  ...
  //form上传
  const bucketname = "image-paile"; //服务名
  const username = "paileupyun"; //操作员账号
  const password = "paileupyun"; //操作员密码
  let str = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o'];
  let imgCode = new Date().getTime() + parseInt(Math.random() * 100).toString() + str[Math.floor(Math.random() * 15)] + str[Math.floor(Math.random() * 15)];
  const save_key = "/paile8vx/" + imgCode + "{.suffix}";
  const url = "http://v0.api.upyun.com/" + bucketname;
  var file = fileName;
